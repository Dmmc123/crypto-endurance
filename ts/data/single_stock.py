...
class SingleAssetDataCollector(BaseModel):
    ticker_symbol: str
    fred_api_key: str
    ...
    def collect(self, world_series_idx: list[str], output_dir: str) -> None:
        ...
        features = SingleAssetDataCollector._enrich_indicators(features)
        features = self._enrich_worldwide_data(features, world_series_idx)
        features = SingleAssetDataCollector._postprocess(features)
...
def main():
    ...
    collector = SingleAssetDataCollector(ticker_symbol=args.ticker, fred_api_key=os.environ["fred-api-key"])
    collector.collect(
